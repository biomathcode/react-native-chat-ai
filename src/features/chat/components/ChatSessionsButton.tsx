...
export function ChatSessionsButton({
  ...
  const travelDistance = Math.max(
    0,
    screenWidth - leftOffset * 2 - CONTROL_SIZE
  );
    ...
    transform: [
      {
        translateY: interpolate(
          Math.max(progress.value, iconProgress?.value ?? 0),
          [0, 1],
          [0, 7]
        ),
      },
