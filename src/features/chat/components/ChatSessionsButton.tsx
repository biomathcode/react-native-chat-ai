...
export function ChatSessionsButton({
  ...
  const topLineStyle = useAnimatedStyle(() => ({
    transform: [
      ...
      { rotateZ: `${interpolate(progress.value, [0, 1], [0, 45])}deg` },
    ],
  }));
  const middleLineStyle = useAnimatedStyle(() => ({
    opacity: interpolate(progress.value, [0, 0.5, 1], [1, 0.2, 0]),
    transform: [{ scaleX: interpolate(progress.value, [0, 1], [1, 0.2]) }],
  }));
  ...
  return (
    <Animated.View style={[styles.sessionsButtonFrame, { top: topOffset }, buttonPositionStyle]}>
      <Pressable
        accessibilityLabel={isOpen ? 'Close chat sessions' : 'Open chat sessions'}
